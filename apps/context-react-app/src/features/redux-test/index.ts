...
import { CTX_REDUX_ASPECT } from '@unvrse/ctx-redux-aspect';
...
const configure: ConfigureFunction = (ctx, options= {}) => {
  ...
  // routes
  ctx.use(CTX_REACT_ROUTER_ASPECT.TYPE_ROUTE, 'redux', { path: '/redux', exact: true, component: 'CounterPage' });

  // menu items
  ctx.use(CTX.TYPE_CONFIG, 'ui.menu.redux', { title: 'Redux', to: '/redux' }).tag('ui.menu');
}
